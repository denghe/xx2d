...
namespace wasm3 {
    ...
    public:
        ...
        void* get_base_addr() {
            return m3_GetMemory(&*m_runtime, 0, 0);
        }

        void* get_true_addr(void* p) {
            if constexpr (sizeof(void*) > 4) {
                if (((size_t)p & 0xFFFFFFFF00000000u) == 0xCCCCCCCC00000000u) {
                    return (void*)((uint32_t)p + m3_GetMemory(&*m_runtime, 0, 0));
                }
            }
            return p;
        }
    ...
    public:
        ...
        Ret call(Args... args) {
            ...
            M3Result res = m3_Call(m_func, sizeof...(args), arg_ptrs.data());
            detail::check_error(res);
