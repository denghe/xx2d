...
struct TexturePackerConfig {
    ...
    XX_FORCEINLINE static bool LineToBool(std::string_view& line, size_t const& numSpaces) {
	    return line[numSpaces + 1] == 't';
    }

    template<typename T>
    XX_FORCEINLINE static void CommaSplitFillVector(std::vector<T>& vs, std::string_view const& line) {
	    vs.clear();
	    T i;
	    size_t offset = 0;
    LabBegin:
