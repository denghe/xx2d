...
namespace xx {
	...
	struct AffineTransform {
		...
		inline static AffineTransform Make(XY const& pos, XY const& anchorSize, XY const& scale, float const& radians) {
			auto x = pos.x;
			auto y = pos.y;
		...
		inline AffineTransform MakeConcat(AffineTransform const& t) const {
			return { a * t.a + b * t.c, 
				a * t.b + b * t.d,
				c * t.a + d * t.c, 
				c * t.b + d * t.d,
		...
		inline AffineTransform MakeInvert(AffineTransform const& t) {
			float de = 1 / (t.a * t.d - t.b * t.c);
			return { de * t.d, -de * t.b, -de * t.c, de * t.a,
				de * (t.c * t.ty - t.d * t.tx), de * (t.b * t.tx - t.a * t.ty) };
		}

		inline AffineTransform& Translate(XY const& pos) {
			tx += a * pos.x + c * pos.y;
			ty += b * pos.x + d * pos.y;
			return *this;
		}

		inline AffineTransform& Scale(XY const& scale) {
			a *= scale.x;
			b *= scale.x;
			c *= scale.y;
			d *= scale.y;
		...
		inline AffineTransform& Rotate(float const& radians) {
		...
		inline XY Apply(XY const& point) const {
