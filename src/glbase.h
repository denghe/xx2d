...
struct GLBase {
	...
	void EndDraw(Texture& t, size_t siz);

	// 内部函数. 绘制一批 sprite. 要求使用相同 shader, texture, siz 不得超过 batchSize
	void DrawSpriteBatch(Sprite* ptr, size_t siz);
	void DrawSpriteBatch(Sprite** ptr, size_t siz);

	// 绘制一批 sprite. 无特殊要求。自动分组调用 DrawSpriteBatch
	void DrawSprites(Sprite* ptr, size_t siz);
	void DrawSprites(Sprite** ptr, size_t siz);
	void DrawSprites(xx::Shared<Sprite>* ptr, size_t siz);
};
