...
namespace PhysicsCircles {
	...
	void Circle::Init(xx::SpaceGridC<Circle>* const& grid_, int32_t const& x, int32_t const& y, int32_t const& r) {
		...
		border->FillCirclePoints({ 0,0 }, radius, {}, Scene::numCircleSegments).SetPosition({ (float)_sgcPos.x, (float)-_sgcPos.y });
	}

	void Circle::Update(xx::Rnd& rnd) {
		int foreachLimit = Scene::foreachLimit, numCross{};
		xx::Pos<> v{};
		...
		_sgc->Foreach9NeighborCells<true>(this, [&](Circle* const& c) {
			...
			// cross?
			if (r12 > p12) {
				auto a = xx::GetAngle<(Scene::maxDiameter * 2 >= 1024)>(cxy, txy);
				auto inc = xx::Rotate(xx::Pos<>{ Scene::speed* r12 / p12, 0 }, a);
				v += inc;
				++numCross;
			}
			}, &foreachLimit);
		...
		if (numCross) {
			...
			if (v.IsZero()) {
				...
				auto inc = xx::Rotate(xx::Pos<>{ Scene::speed, 0 }, a);
				pos += inc;
			}
			// move by v?
			else {
				// speed scale limit
				if (numCross > Scene::speedMaxScale) {
					numCross = Scene::speedMaxScale;
				}
				auto a = xx::GetAngleXY(v.x, v.y);
				auto inc = xx::Rotate(xx::Pos<>{ Scene::speed* numCross, 0 }, a);
				pos += inc;
			}

			// map edge limit
			if (pos.x < 0) pos.x = 0;
			else if (pos.x >= Scene::maxX) pos.x = Scene::maxX - 1;
			if (pos.y < 0) pos.y = 0;
			else if (pos.y >= Scene::maxY) pos.y = Scene::maxY - 1;
		...
		if (timePool >= 1.f / 60) {
			...
			if (eg.Pressed(xx::Mbtns::Right)) {	// erase
				...
				grid.Foreach9NeighborCells(idx, [&](Circle* const& c) {
					auto rr = (c->radius + Scene::maxRadius) * (c->radius + Scene::maxRadius);	// mouse circle radius == maxRadius
